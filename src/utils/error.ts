...
export function isEthersError(error: unknown): error is EthersError {
  if (typeof error === 'object' && error && 'reason' in error && 'code' in error && 'message' in error) return true
  return false
}

export function isMetaMaskError(error: unknown): error is MetaMaskError {
  if (typeof error === 'object' && error && 'code' in error && 'message' in error) return true
  return false
}
